...
const CODEX_NOTIFY_SH_SCRIPT = [
  ...
  "case \"$TAB_ID\" in *[![:space:]]*) ;; *) exit 0 ;; esac",
  ...
  "json_string_field() {",
  "  FIELD=\"$1\"",
  "  printf \"%s\" \"$RAW_PAYLOAD\" | awk -v field=\"$FIELD\" '",
  "    BEGIN { needle = \"\\\"\" field \"\\\"[[:space:]]*:[[:space:]]*\\\"\" }",
  "    match($0, needle) {",
  ...
  "  PREVIEW=$(json_string_field \"last-assistant-message\")",
  "  if [ -n \"$PREVIEW\" ]; then",
  "    PREVIEW_ESCAPED_WHITESPACE=\"true\"",
  "  fi",
  "  THREAD_ID=$(json_string_field \"thread-id\")",
  "  TURN_ID=$(json_string_field \"turn-id\")",
  "  CWD_VALUE=$(json_string_field \"cwd\")",
  "fi",
  ...
  "",
  "printf '{\"v\":1,\"eventId\":\"%s\",\"providerId\":\"%s\",\"tabId\":\"%s\",\"envLabel\":\"%s\",\"threadId\":\"%s\",\"turnId\":\"%s\",\"cwd\":\"%s\",\"sqliteHome\":\"%s\",\"preview\":\"%s\",\"previewEscapedWhitespace\":%s,\"timestamp\":\"%s\"}\\n' \\",
  ...
  "  \"$(json_escape \"$TURN_ID\")\" \\",
  "  \"$(json_escape \"$CWD_VALUE\")\" \\",
  "  \"$(json_escape \"$SQLITE_HOME\")\" \\",
  "  \"$(json_escape \"$PREVIEW\")\" \\",
...
const CODEX_NOTIFY_PS1_SCRIPT = [
  ...
  "$SqliteHome = [string]$env:CODEX_SQLITE_HOME",
  "",
  "$RawPayload = \"\"",
  ...
  "  try { $PayloadObj = $RawPayload | ConvertFrom-Json -ErrorAction Stop } catch {}",
  "}",
  "",
  "$Preview = \"\"",
  ...
  "  }",
  "}",
  "if ([string]::IsNullOrWhiteSpace($Preview) -and $null -ne $PayloadObj) {",
  "  try { $Preview = [string]$PayloadObj.\"last-assistant-message\" } catch {}",
  "}",
  ...
  "  try { $ThreadId = [string]$PayloadObj.\"thread-id\" } catch {}",
  "  try { $TurnId = [string]$PayloadObj.\"turn-id\" } catch {}",
  "  try { $CwdValue = [string]$PayloadObj.cwd } catch {}",
  "}",
  "",
  "try {",
...
const CODEX_HOOK_JS_SCRIPT = [
  ...
  "if (!tabId.trim()) {",
  "  try { fs.writeSync(1, JSON.stringify({ continue: true, suppressOutput: true })); } catch {}",
  "  process.exit(0);",
  "}",
  ...
  "const sqliteHome = String(process.env.CODEX_SQLITE_HOME || '');",
  "",
  "function readStdin() {",
  "  try {",
  ...
  "const threadId = trimText(input.session_id || input.sessionId);",
  "const turnId = trimText(input.turn_id || input.turnId);",
  "const cwd = trimText(input.cwd);",
